<?php
function createHeader($fpdf,$kdsubutir){
    $fpdf->AddPage();
    $fpdf->setXY(10,10);
    ...
    $fpdf->Cell(30,8,"Organoleptik","0","0","C");
    ...
    foreach($rincian as $rinci){
        ...
        $fpdf->SetFont("Times","",7);
        if($c%2===1){
            createHeader($fpdf,$header->kdsubutir);
            $fpdf->SetXY(10,25);
        }else{
            $fpdf->SetXY(10,100);
        }
        ...
        $fpdf->narasiPresenOrganoleptik($c,$rinci);
        $fpdf->narasiOrganoleptik($c,$rinci);

        $fpdf->narasiPenilaianKlinis($c,$rinci);
        $fpdf->penetapanDiagnosaOrganoleptik($c,$rinci);
    }
    $fpdf->Output();
?>
