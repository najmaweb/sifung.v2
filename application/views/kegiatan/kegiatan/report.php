...
function PageHeader($text)
{
    ...
    $this->Cell(90,6,$text,'1',0,'C');
    $this->Ln();
}
function PageHead($text)
{
    $this->SetFont('Times','',16);
    $this->Cell(200,6,$text,'0',0,'C');
    $this->Ln();
}
...
$data = array(
    ...
    array("","a. No dan Tgl Surat Tugas",":",$header->nops.' / '.$sd->mysqltoid($header->tgops)),
    array("","b. Pemberi Tugas",":",$header->pemberi),
    array("4.","Pelaksanaan",":",""),
    array("","a. Lokasi",":",$header->dasar),
    array("","b. Waktu",":",$header->dasar),
    array("","c. Jam",":",$header->dasar),
    array("5.","Hasil Pelaksanaan",":",$header->ringkasan),
);
$pdf->AddPage();
$pdf->headerSetting();
$pdf->PageHeader('Formulir '.$header->kdbutir.'/'.$header->pelaksana.'/'.$header->id);
$pdf->Ln();
...
$pdf->butirTable(
    array(
        array("1","1 april 2022","1224","Bla bla")
    )
);
$pdf->Output();
?>
