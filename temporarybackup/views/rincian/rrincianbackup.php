...
function butirTable($data)
{
    ...
    $c = 1;
    $uraiana = "Memperhatikan hasil rekam medis terkait dengan informasi pemilik, informasi negara/daerah asal hewan, informasi jenis hewan, status present dan dokumen lain yang menyertai \n";
    $uraiana.= "Nama Pemilik \n";
    $uraiana.= "Jenis Hewan \n";
    $uraiana.= "Daerah/negara asal hewan \n";
    $uraiana.= "Alat angkut:jenis, kondisi \n";
    $uraiana.= "Hasil rekam status present: ciri-ciri hewan, nafsu makan dan minum, keadaan berdiri hewan, kondisi bulu,adanya discharge,adanya eksudat \n";
    $uraiana.= "Anamnesa: riwayat sakit/vaksinasi \n";

    foreach($data as $row)
    {
        $this->Cell($w[0],6,$c++,'1',0,'R');
        $this->Cell($w[1],6,$row[1],'1',0,'C');
        $this->Cell($w[2],6,$row[0],'1',0,'C');
        $this->MultiCell($w[3],6,$uraiana,1,'L');
        //$this->Ln();
    }
    $this->AddPage();
}
